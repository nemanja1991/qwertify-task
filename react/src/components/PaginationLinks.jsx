          ...
          >
            ...
            {meta.links && meta.links.map((link, ind) => (
              <a
                ...
                aria-current="page"
                className={
                  "relative z-10 inline-flex items-center px-4 py-2 text-sm font-medium hover:bg-indigo-300 "
                  + (link.url === null ? ' text-indigo-500  text-opacity-25 hover:!bg-white ' : '')
                  + (ind === 0 ? 'rounded-l-md ' : '')
                  + (ind === meta.links.length - 1 ? 'rounded-r-md ' : '')
                  + (link.active ? ' border border-indigo-500  text-indigo-600 ' : '')
                }
